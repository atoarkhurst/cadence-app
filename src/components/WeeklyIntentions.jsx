...
export default function WeeklyIntentions() {
    ...
    useEffect(() => {
        ...
        const fetchSavedIntentions = async () => {
            try {
                const q = query(collection (db, "weeklyIntentions"), orderBy("createdAt", "desc"));
    ...
    const handleSubmit = async () => {
        if (intentions.some((i) => !i.label.trim())) return alert("Fill all 3 first 🙂");
        ...
        try {
            ...
            alert("Intentions saved to Firestore - good luck this week!");
        } catch (error) {
    ...
    return (
        <div className="w-full max-w-md rounded-xl bg-white shadow p-6 space-y-6">
            ...
            <div className="pt-6">
                <h3 className="text-lg font-semibold mb-2 text-center">Past Intentions</h3>
